...
questions = {'Будапешт': 'Этот город в Венгрии был образован в результате слияния 3-х городов в 1873.',
             ...
             'Брест': 'Во Франции и в Беларуси есть города с одинаковым названием. Любопытно, что их освободили от нацистов в один день — 28 июля 1944 года.',
             ...
             'Хум': 'Самый маленький город в мире. Его население колеблется от 17 до 25 человек.',
             'Мехико': 'Самый большой по протяженности город. Что бы преодолеть его по прямой придется пройти более 200 километров.',
             'Банконг': 'Этот город имеет самое длинное название, однако имеется и короткое - ...',
             'Стокгольм': 'Единственная столица над которой раздрешено летать на воздушных шарах - ...'}
...
def dialog(res, req):
    ...
    if req['session']['new']:
        res['response'][
            'text'] = 'Привет! Сейчас мы сыграем в викторину, где тебе предстоит отгадать город по факту о нем. Напиши "Начать" для начала игры.'
        ...
        if sessionStorage[user_id]['game_started'] and req['request']['command'] == 'Помощь':
            ...
            Что бы продолжить ответь на предыдущий вопрос или начни сначала!'''
            ...
            question, answer, answers = generate_question(sessionStorage[user_id]['guessed'])
            print(sessionStorage[user_id]['guessed'])
            sessionStorage[user_id]['count'] += 1
            sessionStorage[user_id]['question'] = question
            sessionStorage[user_id]['answer'] = answer
            sessionStorage[user_id]['answers'] = answers
